...
- 时间：7号上午10:00-12:30
- 地点：东二101
- 流程：每个小组拥有8分钟的展示时间，2分钟的答辩时间。顺序按照小组序号。
  - 因答辩时间有限，小组需要在6日23:59之前确定需要展示的一个challenge（不需要展示所有）
  - 因为时间有限，每个小组在6日23:59之前需要将PPT统一上传至网盘，地址与作业提交地址一致，命名方式为：组名-题名-版本号.zip，如：01-challenge1-v1.zip。助教会统一拷到答辩现场的电脑上，以节省时间。
  - 答辩时间安排在8分钟以内，超出8分钟，将酌情对答辩分数进行扣分。
...
- 最终，每个小组会得到一个平均分（作业的60分），组长需要协调组内成员，根据组员的表现，分配给组员相应的分数，但最终分数的平均值需要等于给定的平均分。
- **平时分40分**（涉及上机练习签到以及上机练习的表现）+**作业分60分**（包括三个报告的分数40分，答辩20分）
